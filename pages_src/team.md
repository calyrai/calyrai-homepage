<header class="site-header">
  <div class="nav-inner">
    <div class="nav-left-row">
      <a href="index.html" class="nav-pill glow-nav">Calyr.ai – platform technology</a>
      <a href="explore.html" class="nav-pill glow-nav">Explore</a>
      <a href="team.html" class="nav-pill glow-nav" aria-current="page">Team</a>
    </div>

    <nav class="nav-links" aria-label="Primary">
      <a href="https://bsky.app/profile/calyrai.bsky.social" class="nav-pill glow-nav">Follow</a>
      <a href="pages/qr_noise.html" class="nav-pill glow-nav">Contact</a>
      <a href="index.html#impressum" class="nav-pill glow-nav">Impressum</a>
    </nav>
  </div>
</header>

<main class="explore-page">
  <section class="explore-shell" aria-label="Team">
    <div class="nexus-intro">
      <section class="nexus-block" aria-label="Team">
        <div class="nexus-kicker">Calyr.ai / Team</div>
        <h1 class="nexus-title">Team</h1>
        <p class="nexus-subtitle">
          Calyrai is a focused effort at the intersection of structural biophysics and machine learning,
          centered on building interpretable systems for scientific data analysis.
        </p>
      </section>

      <section class="nexus-block" aria-label="Contact">
        <h2 class="nexus-h2">Contact</h2>
        <p class="nexus-body">
          For collaboration, research exchange, or hiring discussions:
          <a href="mailto:rupert.tscheliessnig@calyr.ai">rupert.tscheliessnig@calyr.ai</a>
        </p>
      </section>

      <section class="nexus-block" aria-label="Collected">
        <h2 class="nexus-h2">Collected</h2>
        <p class="nexus-body">Hearts you saved on this device.</p>
        <div id="collection-list" class="collection-list" aria-live="polite"></div>
      </section>
    </div>
  </section>
</main>

<footer id="impressum" class="impressum">
  <div class="impressum-inner">
    <h2 class="impressum-title">Impressum</h2>
    <p>
      <strong>CalyrAI</strong> is a private research project by Rupert Gelisnig (Vienna).
    </p>
    <p>
      It is currently not a company, involves no commercial activity, offers no services
      and sells nothing. All content is provided solely for scientific reflection,
      conceptual exploration and academic exchange.
    </p>
    <p class="impressum-contact">
      Contact:
      <a href="mailto:rupert.tscheliessnig@calyr.ai">rupert.tscheliessnig@calyr.ai</a>
    </p>
  </div>
</footer>

<footer class="site-footer">
  © 2026 Calyr.ai™ — All rights reserved.
</footer>

<script src="js/nav_autohide.js"></script>
