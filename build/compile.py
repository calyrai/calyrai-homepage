...
def _sync_company_expertise_page(project_root: Path) -> None:
    ...
    if portrait.get("src"):
        ...
        portrait_html = (
            ...
            f'<img class="hero-portrait__glimmer" src="{portrait_src}" alt="" aria-hidden="true" />'
...
def _sync_research_system(project_root: Path) -> dict[str, Any]:
    ...
    evidence_flow = evidence.get("flow", []) if isinstance(evidence.get("flow"), list) else []
    evidence_invariants = evidence.get("invariants", []) if isinstance(evidence.get("invariants"), list) else []
    evidence_flow_html = "".join(
        f'<article><span>{escape(str(x.get("order", "")))}</span><p class="swiss-system-label">{escape(str(x.get("system", "")))}</p><h3>{escape(str(x.get("name", "")))}</h3><p>{escape(str(x.get("rule", "")))}</p></article>'
        for x in evidence_flow if isinstance(x, dict)
    )
