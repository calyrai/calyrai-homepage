...
import ReactFlow, { Background, Controls, Handle, MarkerType, MiniMap, Position, useNodesState } from "reactflow";
...
function BrixFlow({ concepts }) {
  ...
  const initialIdeaNodes = useMemo(() => concepts.map((item,index) => ({ id:item.id, type:"idea", position:{x:(index%3)*310,y:Math.floor(index/3)*175}, data:{...item,color:accentMap[item.accent]||"#00c7ff"} })), [concepts]);
  const initialStepNodes = useMemo(() => active.flow.map((label,index) => ({ id:`${active.id}-${index}`, type:"step", position:{x:(index%4)*225,y:Math.floor(index/4)*135}, data:{label,index:index+1,color:accentMap[active.accent]||"#00c7ff"} })), [active]);
  const [ideaNodes, setIdeaNodes, onIdeaNodesChange] = useNodesState(initialIdeaNodes);
  const [stepNodes, setStepNodes, onStepNodesChange] = useNodesState(initialStepNodes);
  ...
  useEffect(() => setIdeaNodes(initialIdeaNodes), [initialIdeaNodes, setIdeaNodes]);
  useEffect(() => setStepNodes(initialStepNodes), [initialStepNodes, setStepNodes]);
  ...
  return <div className="brix-flow-system">
    <section className="brix-flow-map"><header><span>01 / Idea index</span><strong>Select an independent surrogate</strong></header><div className="brix-flow-canvas">
      <ReactFlow nodes={ideaNodes} edges={[]} nodeTypes={nodeTypes} onNodesChange={onIdeaNodesChange} onNodeClick={(_,node)=>setActiveId(node.id)} fitView minZoom={0.55} maxZoom={1.35} proOptions={{hideAttribution:true}} {...interactionProps}><Background variant="dots" gap={22} size={1.2} color="#343a45"/><Controls showInteractive={false}/><MiniMap nodeColor={(node)=>node.data.color} maskColor="rgba(5,7,11,.78)"/></ReactFlow>
    </div></section>
    <section className="brix-flow-detail" style={{"--idea-accent":accentMap[active.accent]||"#00c7ff"}}><div className="brix-flow-copy"><span>02 / {active.domain}</span><h3>{active.title}</h3><p>{active.summary}</p><dl><dt>Public view</dt><dd>Schematic concept only</dd><dt>Maturity</dt><dd>{active.maturity}</dd><dt>License boundary</dt><dd>{active.license}</dd></dl></div><div className="brix-step-canvas">
      <ReactFlow nodes={stepNodes} edges={stepEdges} nodeTypes={nodeTypes} onNodesChange={onStepNodesChange} fitView minZoom={0.55} maxZoom={1.35} proOptions={{hideAttribution:true}} {...interactionProps}><Background variant="dots" gap={22} size={1.2} color="#343a45"/><Controls showInteractive={false}/></ReactFlow>
    </div></section>
  </div>;
}
