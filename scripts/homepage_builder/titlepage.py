...
{font_link}{gsap_script_tag}  <style>
		...
		* {{ box-sizing: border-box; }}
		...
		.study-section {{ position: relative; border: 1px solid rgba(255,255,255,0.64); background: transparent; overflow: hidden; isolation: isolate; transition: transform 240ms ease, box-shadow 240ms ease, flex-basis 240ms ease, width 240ms ease, min-height 240ms ease, background 240ms ease, border-color 240ms ease, opacity 200ms ease, filter 200ms ease; }}
		.study-section::before {{ content: ''; position: absolute; inset: 0; z-index: 1; pointer-events: none; background: linear-gradient(165deg, rgba(0, 0, 0, 0.52) 0%, rgba(0, 0, 0, 0.68) 100%); }}
		.study-section.is-open::before {{ background: linear-gradient(165deg, rgba(0, 0, 0, 0.62) 0%, rgba(0, 0, 0, 0.78) 100%); }}
		.study-section .living-surface {{ z-index: 0 !important; }}
