// js/pr_iq_viewer.js
// P(r) & I(q) – Wavelet model + drag&drop experimental data
// plus hook: window.updatePrFromPDB({ r, P }) from the PDB viewer.
...
// allow P(r) injection from the PDB viewer
...
function drawIq() {
  ...
  const eps = 1e-14;
  const logQmodel = qGrid.map(q => Math.log10(Math.max(q, eps)));
  const baseLogImodel = Iq.map(I => Math.log10(Math.max(Math.abs(I), eps)));
  const scaledLogImodel = baseLogImodel.map(li => li + IqScaleLog);

  // axis ranges: if experimental data present, use ONLY that
  let xMin, xMax, yMin, yMax, yVals;
  if (expIqLog) {
    const logQexp = expIqLog.logQ;
    xMin = Math.min(...logQexp);
    xMax = Math.max(...logQexp);
  ...
  // experimental I(q): white (with current offset)
...
// ============================
// Drag & Drop parsing (P(r), I(q) files)
// ============================
